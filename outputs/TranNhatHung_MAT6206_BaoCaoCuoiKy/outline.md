...
1. Định nghĩa trạng thái **Default/Foreclosure**: `delinquency_status ≥ '06'` (≥180 ngày) HOẶC `RA` HOẶC `zero_balance_code ∈ {02,03,09}`, kèm quy tắc "sticky absorbing" (Phương án C, ~4.921 sự kiện trên 150.000 khoản vay 3 vintage 2016–2018).
2. Khoản vay **trả hết trước hạn (prepaid)**: thêm trạng thái hấp thụ thứ hai "Prepaid" (không censoring) — sáu trạng thái, $R$ có hai cột (Phương án B, xem Ch.2 mục 2.5.5).
3. Kỳ hạn (horizon) dùng cho backtest ở 4.5: **chỉ $H=12$ tháng** (Phương án A — không dùng thêm $H=24$).
