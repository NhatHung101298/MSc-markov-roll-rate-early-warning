"""Smoke test cho scripts/utils/markov.py bang ma tran toy (khong dung du lieu that).

Chay: python scripts/00_smoke_test_markov.py
"""
...
# Toy chain 4 trang thai: A(0), B(1) tam thoi; Default(2), Prepaid(3) hap thu.
...
# N, B tinh tay (phan so mau 29): N=[[70/29,30/29],[20/29,50/29]], B=[[16/29,13/29],[17/29,12/29]]
...
# 1. mle_transition_matrix: dem toy khop dung P (dung P*100 lam n_ij)
...
# 2. fundamental_matrix + absorption_probabilities khop tinh tay
...
# 3. pd_finite_horizon: H lon phai hoi tu ve B (vi Q^H -> 0)
...
# 4. Chapman-Kolmogorov: P^2 . P^1 == P^3
...
# 5. stationary_distribution: nghiem phai thoa man pi P = pi va tong = 1
...
# 6. forecast_distribution: mu0 tai A, du bao 1 buoc phai khop hang A cua P
...
# 7. chi2_homogeneity_test: 2 giai doan toy, khong crash, p-value hop le
...
# 8. lr_test_markov_order: sinh n_ijk toy tu counts (gia dinh bac 1 dung), khong crash
k = len(STATES)
...
check("lr_test_markov_order statistic >= 0 (xap xi, du sai so so hoc)", stat_o >= -1e-6)

print()
if failures:
    print(f"KET QUA: {len(failures)} FAIL -- {failures}")
    sys.exit(1)
print("KET QUA: TOAN BO PASS")
sys.exit(0)
